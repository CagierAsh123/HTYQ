...
【事件链防滚雪球规则（强制）】
6. 已有事件链的级别(level)不得升级，除非本轮对话正文中出现了明确的新冲突行为（如新的打斗、新的谋杀、新的挑衅）。仅凭"时间流逝"或"局势发酵"不能作为升级理由。
7. 事件链的规模（如参与人数、波及范围）不得凭空扩大。不能把"两人的口角"升级为"团体的械斗"，不能把"街区冲突"升级为"全城动荡"，除非正文中有相应的新事件支撑。
8. 禁止凭空创建新的事件链。每个新事件链必须能在【最近对话正文】中找到直接触发原因。没有触发原因就不创建。
9. 事件链可以自然衰退：若无新的推动因素，已挂起或停滞的事件链应逐渐降级或维持现状，而非升级。
`;
    ...
    async function buildEvolutionPrompt() {
        ...
        const prompt = `${priorityRule}\n${rules}\n${worldContext}\n${chatHistory}\n当前世界状态：\n轮次：${s.round}\n时间：${s.worldTime || '未知'}\n世界摘要：${s.worldDigest}\n整体氛围：${s.overallAtmosphere}\n驱动事件：${s.drivingEvent}\n星象：${s.astrology}\n治安状况：${s.securityStatus}\n市民情绪：${s.citizenMood}\n直接接触层：${s.directLayer}\n近距离层：${s.nearLayer}\n远距离层：${s.farLayer}\n事件链：${JSON.stringify(s.events.slice(0,5))}\n团体：${JSON.stringify(s.factions.slice(0,5))}\n流言：${JSON.stringify(s.rumors.slice(0,5))}\n声誉：${JSON.stringify(s.reputation)}\n玩家当前余额：${currencyAmount} ${currencyName}\n即将发生的日程：${JSON.stringify(s.upcomingSchedules)}\n随机事件：${JSON.stringify(s.randomEvents)}\n请根据上述角色设定和世界书（仅作参考），以当前对话正文为准，推演世界新状态，以JSON格式返回，必须包含以下字段：\nworld_time, world_digest, overall_atmosphere, driving_event, citizen_mood, security_status, astrology, direct_layer, near_layer, far_layer, upcoming_schedules(数组，每个元素含time,event,involved,potentialImpact), reputation(四个维度), reputation_change(字符串), rumors(数组，每个对象含content,type,scope,credibility,source,impact,heat), events(数组，每个对象含name,level,stage,currentRound,totalRounds,desc,trigger), factions(数组，每个对象含name,region,current_goal,progress,cohesion,resources,attention_to_user,core_character), faction_relations(数组，每个对象含factionA,factionB,relation,level,trend), economy(包含currencyName, currencyAmount填变化量正负值-不是余额, marketTrend, keyResources数组(每个含name,status), fundsStatus(自然语言), economyVisibility对象(behavior,visible,witnesses,rumorGenerated)), blackMarket(数组), accidents(数组), active_contact(可选), recent_actions(数组), memory_summary(字符串), causal_chain(数组), random_events(数组), power_peaks(数组), internal_messages(数组), secret_box(对象), character_states(数组), diplomatic_events(数组), pending_foreshadowing(数组), key_values_memo(字符串), round_focus(字符串), cross_region_memo(字符串), blood_feud_memo(字符串)。\n【强制规则】economy.currencyAmount 只填本轮变化量。例如+50表示赚了50、-20表示花了20、0表示无变化。禁止填当前总余额，禁止凭空造大额收入。\n只返回JSON，不要有其他文字。`;
        return prompt;
    }
